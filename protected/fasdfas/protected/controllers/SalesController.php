...
//		fwrite($fh, ""."\r\n");
     ...
     //   $hit = count($data_detail['quantity_purchased']);
		...
		')
		->from('sales s, sales_items si,sales_payment sp,items i')
		->where("i.id = si.item_id and  i.category_id != 4 and  s.id = si.sale_id and s.id = $id and s.id = sp.id")
		->group("s.id")
		->queryRow();
...
//        fwrite($fh, $pembatas . "\r \n");
        ...
        if (isset($_GET['Sales']['date'])) 
			...
			sum(((si.item_price*si.quantity_purchased)+(si.item_tax)-(si.item_discount*(si.item_price*si.quantity_purchased)/100)))  sale_total_cost,
			 ...
			 where 
			  ...
			  i.id = si.item_id and i.category_id != 4 
			  
			 s.id = si.sale_id and date(s.date)='$date' and s.status=1 and inserter = u.id  and inserter = $user->id   group by s.id  ";
			$dataProvider = new CSqlDataProvider($sql, array(
				'totalItemCount'=>$count,
				'sort'=>array(
				'attributes'=>array(
            ...
            $summary = Yii::app()->db->createCommand()
					...
					si.sale_id = s.id and date(s.date)=:date AND status=:status and inserter =:ins ', array(':date' => $date, ':status' => 1,"ins"=>$user->id))
			...
			sum(((si.item_price*si.quantity_purchased)+(si.item_tax)-(si.item_discount*(si.item_price*si.quantity_purchased)/100)))  sale_total_cost,
			 ...
			 and s.id = si.sale_id and date(s.date)='$date' and s.status=1 and inserter = u.id  group by s.id  ";
			$dataProvider = new CSqlDataProvider($sql, array(
				'totalItemCount'=>$count,
				'sort'=>array(
				'attributes'=>array(
	...
	$this->slg =  Branch::model()->findByPk(1)->slogan;
		...
		->where("date(date) = '".$date."' and sales.status = 1 and items.category_id !=5")
		->group('item_id')
