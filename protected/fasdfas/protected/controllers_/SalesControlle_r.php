...
class SalesController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('delete','index', 'view', 'bayar', 'load', 'void','Getsaleid','hanyacetak','cashreport','CetakReport','Pindahmeja','sessid','Uservoid','Cetakrekap','Export','Salesmonthly','Outletreport','Salesoutletmonthly','Salescashmonthly','detailitems','ex','printData'),
                'users' => array('*'),
            ),
	...
	public function actionPrintData(){
		...
		->where("
				...
				")
		->group('sales_items.item_id,sales.id	')
		->order('sales.id asc')
		->queryAll();
...
//        $data['si'] = $si;
    ...
    public function actionBayar() {
		...
		//mencari QTY		
			...
			// $pembayaran->sale_id = $sales->id;
			// $pembayaran->cara_bayar =  $data['paidwith2'];
			// $pembayaran->total =  $data['payment2'];
			// $pembayaran->save();
            ...
            $sales->status = $data['status'];
                ...
                foreach ($data_detail as $detail){
					...
					Pembayaran::model()->deleteAllByAttributes(array('sale_id' => $sales->id));
                			
					$pembayaran = new Pembayaran();
					$pembayaran->sale_id = $sales->id;
					$pembayaran->cara_bayar =  $data['paidwith'];
					$pembayaran->total =  $data['payment'];
					$pembayaran->save();
					$pembayaran = new Pembayaran();
					$pembayaran->sale_id = $sales->id;
					$pembayaran->cara_bayar = $data['paidwith2'];
	...
	public function actionCashReport(){
		...
		if (isset($_GET['Sales']['date'])) {
			...
			$row = Yii::app()->db->createCommand()
					...
					sales.id,
					date,
					if(sale_id<>0,sum(total),sum(sale_total_cost)) as "total_cost",
					if(sale_id<>0,sum(if (pembayaran.cara_bayar=1,total,0)),if(paidwith_id=1,sum(sale_total_cost),0)) as "netcash",
					if(sale_id<>0,sum(if (pembayaran.cara_bayar=3,total,0)),if(paidwith_id=3,sum(sale_total_cost),0)) as "BCA",
					if(sale_id<>0,sum(if (pembayaran.cara_bayar=4,total,0)),if(paidwith_id=4,sum(sale_total_cost),0)) as "mandiri",
					if(sale_id<>0,sum(if (pembayaran.cara_bayar=5,total,0)),if(paidwith_id=5,sum(sale_total_cost),0)) as "niaga",
					if(sale_id<>0,sum(if (pembayaran.cara_bayar=12,total,0)),if(paidwith_id=12,sum(sale_total_cost),0)) as "compliment",
					if(sale_id<>0,sum(if (pembayaran.cara_bayar=99,total,0)),if(paidwith_id=99,sum(sale_total_cost),0)) as "voucher"					
					')
					->from('sales')
					->leftJoin('pembayaran','sales.id = pembayaran.sale_id')
			...
			$row = Yii::app()->db->createCommand()
					...
					')
					->from('sales')
					->leftJoin('pembayaran','sales.id = pembayaran.sale_id')->where('date(date)=:date and status=1 and inserter=:i', array(':date' => $_GET['Sales']['date'], ':i'=> $user->id))
					->group('sales.date')
					->queryAll();
