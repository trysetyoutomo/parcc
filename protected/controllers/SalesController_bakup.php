    ...
    public function accessRules() {
        return array(
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('printbagihasil','salescashweekly','salesweekly','salesoutletweekly','index', 'view', 'bayar', 'load', 'void','Getsaleid','hanyacetak','cashreport','CetakReport','Pindahmeja','sessid','Uservoid','Cetakrekap','Export','Salesmonthly','Outletreport','Salesoutletmonthly','Salescashmonthly','detailitems','ex','printData'),
                'users' => array('*'),
            ),
...
//        fwrite($fh, $pembatas . "\r \n");
     ...
     */
	 ...
	 public function actionSalescashmonthly(){
		...
		$tot = Yii::app()->db->createCommand()
			->select('date(s.date) tanggal,s.id,s.date,sum(cash)cash,sum(edc_bca)edc_bca,sum(edc_niaga)edc_niag,sum(compliment)compliment,sum(dll)dll,sum(voucher)voucher, SUM(cash+edc_bca+edc_niaga+compliment+dll+voucher) grandtotal	')
	...
	public function actionSalesweekly() {
		...
		if (isset($_GET['Sales']['date']) and isset($_GET['Sales']['tgl']) ) {
			...
			$dataProvider=new CActiveDataProvider('Sales', array(
						'criteria'=>array(
							'condition'=>'status = :stat and inserter =:ins and date(date)>= :date and  date(date)<= :date2 ',
            ...
            $summary = Yii::app()->db->createCommand()
                    ->select('sum(sale_total_cost) stc, sum(sale_discount) sd , sum(sale_sub_total) sst, sum(sale_tax) t, sum(sale_service) svc')
                    ->from('sales ')
                    ->where('status=:status and inserter =:ins and date(date)>= :date and  date(date)<= :date2  ', array(':date' => $tgl,':date2'=>$tgl2, ':status' => 1,"ins"=>$user->id))
                    ->queryRow();
			}
			else{
				$dataProvider=new CActiveDataProvider('Sales', array(
            ...
            $summary = Yii::app()->db->createCommand()
                    ->select('sum(sale_total_cost) stc, sum(sale_discount) sd , sum(sale_sub_total) sst, sum(sale_tax) t, sum(sale_service) svc')
                    ->from('sales ')
                    ->where('status=:status and date(date)>= :date and  date(date)<= :date2   ', array(':date' => $tgl,':date2'=>$tgl2, ':status' => 1))
                    // ->where('date(date)=:date AND status=:status', array(':date' => $_GET['Sales']['date'], ':status' => 1))
                    // ->where("date='".$_GET['Sales']['date']."'")
                    ->queryRow();
