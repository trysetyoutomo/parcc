...
class SalesController extends Controller {
	...
	public function actionSalesmonthly(){
		...
		$tot = Yii::app()->db->createCommand()
			...
			->where("month(date) =  '$month' and year(date)='$year'  and s.status=1  and s.id = si.sale_id")
			->group('date(s.date)')
...
public function actionSalesoutletmonthly(){	
		...
		foreach($itemdata as $key){
			...
			$ulang =  'sum(if(o.kode_outlet='.$key["kode_outlet"].',((si.item_price*si.quantity_purchased)-(si.item_discount*si.item_price/100)) * (o.persentase_hasil / 100),0)) as "'.$key["kode_outlet"].'",';
			$kata_bersih_d = $kata_bersih_d.' '.$ulang;
		}	
		
		$q2 = $kata_bersih_d . 'sum(o.kode_outlet)' ;
		$bersih  = Yii::app()->db->createCommand()
			->select("date(s.date) tgl,sum(((si.item_price*si.quantity_purchased)-(si.item_discount*si.item_price/100))) as sales,sum(((si.item_price*si.quantity_purchased)-(si.item_discount*si.item_price/100)) * (100-o.persentase_hasil) / 100) as ba,".$q2)
...
public function actionSalesoutletweekly(){	
		...
		$q2 = $kata_bersih_d . 'sum(o.kode_outlet)' ;
		$bersih  = Yii::app()->db->createCommand()
			->select("sum(((si.item_price*si.quantity_purchased)-(si.item_discount*si.item_price/100))) as sales,date(s.date) tgl,sum(((si.item_price*si.quantity_purchased)-(si.item_discount*si.item_price/100)) * (100-o.persentase_hasil) / 100) as ba,".$q2)
			->from('sales_items si,outlet o,sales s,items i')
...
//		fwrite($fh, ""."\r\n");
     ...
     //   $hit = count($data_detail['quantity_purchased']);
		...
		$kmb = Yii::app()->db->createCommand()
		->select('
		s.bayar-(sum(((si.item_price*si.quantity_purchased)+(si.item_tax)-(si.item_discount*(si.item_price*si.quantity_purchased)/100))) -sp.voucher) kembali
		')
		->from('sales s, sales_items si,sales_payment sp')
		->where("s.id = si.sale_id and s.id = $id and s.id = sp.id")
		->group("s.id")
		->queryRow();
	...
	public function actionCetakReport($id){
		$saleid = Yii::app()->db->createCommand()
			...
			')
			->from('sales s, sales_items si')
			->where("s.id = si.sale_id and s.id = $_GET[id] ")
			->group("s.id")
			->queryRow();
		// $saleid = Sales::model()->findByPk($_GET['id']);
		$arr_sales = array();
...
//        fwrite($fh, $pembatas . "\r \n");
	...
	public function actionOutletreport(){
		...
		// var_dump($itemdata);
			...
			$q = 's.id,s.date time,'.$kata.'sum(si.item_price*si.quantity_purchased+(s.sale_service/s.total_items)*si.quantity_purchased-(s.sale_discount/s.total_items)*si.quantity_purchased) as total';
    ...
    public function actionIndex() {
        ...
        if (isset($_GET['Sales']['date'])) 
            ...
            $summary = Yii::app()->db->createCommand()
                    ...
                    ->from('sales s, sales_items si')
                    ->where('si.sale_id = s.id and date(s.date)=:date AND status=:status and inserter =:ins ', array(':date' => $date, ':status' => 1,"ins"=>$user->id))
			...
			sum(((si.item_price*si.quantity_purchased)+(si.item_tax)-(si.item_discount*(si.item_price*si.quantity_purchased)/100)))  sale_total_cost,
			 u.username inserter 
			 from sales s,sales_items si , users u 
			 where s.id = si.sale_id and date(s.date)='$date' and s.status=1 and inserter = u.id  group by s.id  ";
			$dataProvider = new CSqlDataProvider($sql, array(
				'totalItemCount'=>$count,
				'sort'=>array(
				'attributes'=>array(
            ...
            $summary = Yii::app()->db->createCommand()
					...
					stc, sum(sale_discount) sd , sum(si.item_price*si.quantity_purchased) sst, sum(item_tax) t, sum(sale_service) svc')
                    ->from('sales s, sales_items si')
                    ->where('s.id = si.sale_id and date(s.date)=:date AND status=:status  ', array(':date' => $date, ':status' => 1))
                    // ->where('date(date)=:date AND status=:status', array(':date' => $_GET['Sales']['date'], ':status' => 1))
                    // ->where("date='".$_GET['Sales']['date']."'")
                    ->queryRow();
	...
	public function actionSalesweekly() {
		...
		if (isset($_GET['Sales']['date']) and isset($_GET['Sales']['tgl']) ) {
			...
			sum(((si.item_price*si.quantity_purchased)+(si.item_tax)-(si.item_discount*(si.item_price*si.quantity_purchased)/100)))  sale_total_cost,
			 u.username inserter 
			 from sales s,sales_items si , users u 
			 where s.id = si.sale_id and date(s.date)>='$tgl' and date(s.date) <= '$tgl2' and s.status=1 and inserter = u.id  and inserter = $user->id   group by s.id  ";
			$dataProvider = new CSqlDataProvider($sql, array(
				'totalItemCount'=>$count,
				'sort'=>array(
				'attributes'=>array(
            ...
            $summary = Yii::app()->db->createCommand()
					...
					stc, sum(sale_discount) sd , sum(si.item_price*si.quantity_purchased) sst, sum(item_tax) t, sum(sale_service) svc')
                    ->from('sales s, sales_items si')
                    ->where('si.sale_id = s.id and date(s.date)>=:date and date(s.date)<=:date2 AND status=:status and inserter =:ins ', array(':date' => $tgl, ':date2'=>$tgl2,':status' => 1,"ins"=>$user->id))
                    ->queryRow();
			}
			else{
			...
			sum(((si.item_price*si.quantity_purchased)+(si.item_tax)-(si.item_discount*(si.item_price*si.quantity_purchased)/100)))  sale_total_cost,
			 u.username inserter 
			 from sales s,sales_items si , users u 
			 where s.id = si.sale_id and date(s.date)>='$tgl' and date(s.date) <= '$tgl2'  and s.status=1 and inserter = u.id  group by s.id  ";
			$dataProvider = new CSqlDataProvider($sql, array(
				'totalItemCount'=>$count,
				'sort'=>array(
				'attributes'=>array(
            ...
            $summary = Yii::app()->db->createCommand()
					...
					stc, sum(sale_discount) sd , sum(si.item_price*si.quantity_purchased) sst, sum(item_tax) t, sum(sale_service) svc')
                    ->from('sales s, sales_items si')
                    ->where('s.id = si.sale_id and date(s.date)>=:date and date(s.date)<=:date2 AND status=:status  ', array(':date' => $tgl,':date2'=>$tgl2 ,':status' => 1))
                    // ->where('date(date)=:date AND status=:status', array(':date' => $_GET['Sales']['date'], ':status' => 1))
                    // ->where("date='".$_GET['Sales']['date']."'")
                    ->queryRow();
	...
	$this->slg =  Branch::model()->findByPk(1)->slogan;
		...
		sum(((si.item_price*si.quantity_purchased)+(si.item_tax)-(si.item_discount*(si.item_price*si.quantity_purchased)/100))) as stc 
		,s.id
		,s.date
		')
		->from('sales s,sales_items si ')
		...
		date(s.date)=:date 
		and  s.status=1 
		and s.id = si.sale_id', array(':date' => $date))
		->queryAll();
		...
		and  s.status=1 
		 and s.inserter=:i
		and s.id = si.sale_id', array(':date' => $date,':i'=> $user->id))
		->queryAll();
		...
		->from('sales s, acp a, users u')
		->where(" s.id = a.sale_id AND s.inserter = u.id AND  s.status = 1  ")
		//sales.inserter = ".$user->id." 
		->queryAll();
		// echo "<pre>";
		// print_r($hutang);
		...
		foreach($payment as $dt1){
			...
			$tmpt['mandiri'] =  "Niaga         :";
			$tmpt['mandirivalue'] = "\r".number_format($dt1['edc_niaga'])."\r\n";
			
			$tmpt['niaga'] =    "Voucher       :";
			$tmpt['niagavalue'] = "\r".number_format($dt1['voucher'])."\r\n";
