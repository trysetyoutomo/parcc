...
class SalesController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('printdata','index', 'view', 'bayar', 'load', 'void','Getsaleid','hanyacetak','cashreport','CetakReport','Pindahmeja','sessid','Uservoid','Cetakrekap','Export','Salesmonthly','Outletreport','Salesoutletmonthly','Salescashmonthly','detailitems','ex','sendmail','routlet'),
                'users' => array('*'),
            ),
	...
	public function actionPrintData(){
		...
		->select('sales.id as sid,sales.date as date,item_name ,item_price, item_discount,nama_outlet,sum(quantity_purchased) as jumlah, item_tax, sum((quantity_purchased*item_price)+item_tax-(quantity_purchased*item_discount)) total	')
		...
		->select('nama_outlet nm, persentase_hasil ps, sum((quantity_purchased*item_price)+item_tax-(quantity_purchased*item_discount))  total , sum((quantity_purchased*item_price)+item_tax-(quantity_purchased*item_discount))  * persentase_hasil /100 as bersih 	')
		->from('sales_items, sales ,items,outlet')
		->where("
				sales.id = sales_items.sale_id and
				sales.status = 1 and
	...
	public function actionRoutlet(){
		$tgl1 = $_GET['tgl1'];
		$tgl2 = $_GET['tgl2'];
		$id = $_GET['id'];
		$dataProvider = Yii::app()->db->createCommand()
		->select('sales.date as date,item_name ,item_price, item_discount,nama_outlet,sum(quantity_purchased) as jumlah, item_tax, sum(item_price+item_tax-item_discount) total	')
		->from('sales_items, sales ,items,outlet')
		->where("
				sales.id = sales_items.sale_id and
				sales.status = 1 and
		...
		$ambilData = Yii::app()->db->createCommand()
		->select('nama_outlet nm, persentase_hasil ps, sum(item_price+item_tax) total , sum(item_price+item_tax-item_discount) * persentase_hasil /100 as bersih 	')
		->from('sales_items, sales ,items,outlet')
		->where("
				sales.id = sales_items.sale_id and
				sales.status = 1 and
	...
	//	->group('item_name')
		...
		$this->render('detailoutlet', array(
			'dataProvider' => $x,
			'data'=>$ambilData
		));
    ...
    {   
        ...
        $message->setBody("<h1>tst123</h1>", 'text/html');                
        $message->addTo('try35u@gmail.com');
        $message->from = 'try.setyoutomo@yahoo.com';   
	...
	public function actionEx(){
		...
		$pdf->LastPage();
		$pdf->Output("C:/Users/try/Dropbox/Food Arena Report [$tglheader - $tgl2header].pdf", "F");
	...
	public function actionDetailitems(){	
		...
		->select('
				...
				item_discount,
				nama_outlet,
				sum(quantity_purchased) as jumlah,
				item_tax, 
				sum(item_price+item_tax-item_discount) total	
				')
		->from('sales_items, sales ,items,outlet')
		->where("
				sales.id = sales_items.sale_id and
	...
	public function actionSalesoutletmonthly(){
		...
		->from('netoutlet,sales')
		->where("
		sales.id=netoutlet.sale_id and
		sales.status=1 and
		...
		//mencari QTY		
            ...
            $sales->status = $data['status'];
                ...
                foreach ($data_detail as $detail){
					...
					$di->quantity_purchased = $detail['quantity_purchased'];
                    $di->item_tax = $detail['item_tax'];
                    $di->item_discount = $detail['item_discount']*$detail['item_price']/100;
                    $di->item_price = $detail['item_price'];
                    // $di->item_discount = $detail['item_discount'];;
					$di->item_service =  1;
					...
					//query untuk mencari laba kotor
					$itc = Yii::app()->db->createCommand()
					->select('si.id,sum((si.item_price*si.quantity_purchased)+si.item_tax-(si.item_discount*si.quantity_purchased)) as total')
					->from('sales s,sales_items as si,items as 	i')
					->where('s.id=si.sale_id and si.item_id= i.id and si.id='.$di->id.'')
					->queryRow();
...
//        fwrite($fh, $pembatas . "\r \n");
	...
	public function actionOutletreport(){
		...
		$jumlah = count($itemdata);
			...
			$bersih = Yii::app()->db->createCommand()
                    ->select('
						sum(if(netoutlet.kode_outlet=1,netoutlet.net_item_outlet,0)) as o1,
						sum(if(netoutlet.kode_outlet=2,netoutlet.net_item_outlet,0)) as o2,
						sum(if(netoutlet.kode_outlet=3,netoutlet.net_item_outlet,0)) as o3,
						sum(if(netoutlet.kode_outlet=4,netoutlet.net_item_outlet,0)) as o4,
						sum(if(netoutlet.kode_outlet=5,netoutlet.net_item_outlet,0)) as o5,
						sum(if(netoutlet.kode_outlet=6,netoutlet.net_item_outlet,0)) as o6,
						sum(if(netoutlet.kode_outlet=7,netoutlet.net_item_outlet,0)) as o7,
						sum(if(netoutlet.kode_outlet=8,netoutlet.net_item_outlet,0)) as o8,
						sum(netoutlet.net_item_comp) as total_comp
											')
                    ->from('netoutlet,sales')
                    ->where('
					date(sales.date)=:date and
					sales.id=netoutlet.sale_id and
					sales.status=1
					', array(':date' => $tgl))
                    ->queryRow();
			...
			$summary = Yii::app()->db->createCommand()
					->select(' 
					sum(if(i.kode_outlet=1,si.item_price*si.quantity_purchased+(s.sale_service/s.total_items)*si.quantity_purchased+si.item_tax-(s.sale_discount/s.total_items)*si.quantity_purchased,0)) as o1,
					sum(if(i.kode_outlet=2,si.item_price*si.quantity_purchased+(s.sale_service/s.total_items)*si.quantity_purchased+si.item_tax-(s.sale_discount/s.total_items)*si.quantity_purchased,0)) as o2,
					sum(if(i.kode_outlet=3,si.item_price*si.quantity_purchased+(s.sale_service/s.total_items)*si.quantity_purchased+si.item_tax-(s.sale_discount/s.total_items)*si.quantity_purchased,0)) as o3,
					sum(if(i.kode_outlet=4,si.item_price*si.quantity_purchased+(s.sale_service/s.total_items)*si.quantity_purchased+si.item_tax-(s.sale_discount/s.total_items)*si.quantity_purchased,0)) as o4,
					sum(if(i.kode_outlet=5,si.item_price*si.quantity_purchased+(s.sale_service/s.total_items)*si.quantity_purchased+si.item_tax-(s.sale_discount/s.total_items)*si.quantity_purchased,0)) as o5,
					sum(if(i.kode_outlet=6,si.item_price*si.quantity_purchased+(s.sale_service/s.total_items)*si.quantity_purchased+si.item_tax-(s.sale_discount/s.total_items)*si.quantity_purchased,0)) as o6,
					sum(if(i.kode_outlet=7,si.item_price*si.quantity_purchased+(s.sale_service/s.total_items)*si.quantity_purchased+si.item_tax-(s.sale_discount/s.total_items)*si.quantity_purchased,0)) as o7,
					sum(if(i.kode_outlet=8,si.item_price*si.quantity_purchased+(s.sale_service/s.total_items)*si.quantity_purchased+si.item_tax-(s.sale_discount/s.total_items)*si.quantity_purchased,0)) as o8,
					sum(si.item_price*si.quantity_purchased+(s.sale_service/s.total_items)*si.quantity_purchased+si.item_tax-(s.sale_discount/s.total_items)*si.quantity_purchased) as total
 ...
 /*       } else {
	...
	$this->slg =  Branch::model()->findByPk(1)->slogan;
		...
		->join('outlet', 'outlet.kode_outlet = items.kode_outlet')
		// ->where('date(date=:date)', array(':date'=>date('2013-03-05')))
		->where("date(date) = '".$date."' and sales.inserter = ".$user->id." ")
		->group('item_id')
		->order('outlet.kode_outlet')
		->queryAll();
		}
        ...
        $temp_data['no_telp'] = $this->set_spacebar("Telp. ".$this->tlp, $total_margin, "tengah");
        // $temp_data['no_nota'] = $this->set_spacebar("No Nota : Belum Bayar", "", "tengah");
        $temp_data['trx_tgl'] = $this->set_spacebar($date, $total_margin, "tengah");
        $pjg_ket = $total_margin - 13;
        // $temp_data['no_meja'] = "Meja   : " . $this->set_spacebar($model['table'], $pjg_ket, "tengah") . "\r";
		$username = Yii::app()->user->name;
		//$user = Users::model()->find('username=:un',array(':un'=>$username));
        ...
        $temp_data['kasir']=   "Kasir  : " . $this->set_spacebar($username                                        , $pjg_ket, "tengah") . "\r";
		...
		foreach ($data as $rows)
		{
			...
			if($rows['nm']!=$ctgr){
				$ctgr = $rows['nm'];
				$wCtgr = "TENANT: ".strtoupper($rows['nm'])."\r\n";
				$tmp4['dept'] = $wCtgr;
