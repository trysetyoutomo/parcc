...
<div class="meja-modal" id="modal">
	...
	$model = Sales::model()->findAllByAttributes(array('status' => 0, 'branch'=>1));
	...
	$x = Sales::model()->find("t.table=$a and t.status = 0");
		if (isset($data[$a])){
			...
			echo '<input title="Perbaharui pada tanggal '.$x->date.'" type="button"  value="' . $a . '" style="background:red;font-color:white" data-dropdown="#dropdown-4" class="table-button" />';
